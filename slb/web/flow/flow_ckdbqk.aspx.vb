...
#End Region
        ...
        '----------------------------------------------------------------
        Private m_objSaveScence As Josco.JSOA.BusinessFacade.IMFlowCkdbqk
        Private m_blnSaveScence As Boolean

        '----------------------------------------------------------------
        '模块接口参数
        '----------------------------------------------------------------
        Private m_objInterface As Josco.JSOA.BusinessFacade.IFlowCkdbqk
        ...
        Private m_objDataSet_DBSJ As Josco.JSOA.Common.Data.FlowData
        ...
        Private Function saveModuleInformation() As String
            ...
            Try
                ...
                '创建对象
                Me.m_objSaveScence = New Josco.JSOA.BusinessFacade.IMFlowCkdbqk
        ...
        Private Function getInterfaceParameters(ByRef strErrMsg As String, ByRef blnContinue As Boolean) As Boolean
            ...
            Try
                ...
                Try
                    ...
                    m_objInterface = CType(objTemp, Josco.JSOA.BusinessFacade.IFlowCkdbqk)
                Catch ex As Exception
                ...
                If Me.IsPostBack = False Then
                    ...
                    Try
                        Me.m_objSaveScence = CType(Session.Item(strSessionId), Josco.JSOA.BusinessFacade.IMFlowCkdbqk)
                    Catch ex As Exception
                        Me.m_objSaveScence = Nothing
                    End Try
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function getModuleData_DBSJ(ByRef strErrMsg As String) As Boolean

            getModuleData_DBSJ = False

            Try
                '创建缺省数据
                If Me.m_objsystemFlowObject.getDubanData(strErrMsg, Me.m_objDataSet_DBSJ) = False Then
                    GoTo errProc
                End If
...
errProc:
        ...
        Private Function showModuleData_Edit(ByRef strErrMsg As String) As Boolean
            ...
            Try
                ...
                Dim strDBSJ As String = ""
                Dim intCount As Integer
                Dim i As Integer
                With Me.m_objDataSet_DBSJ.Tables(strTable).DefaultView
                    intCount = .Count
                    For i = 0 To intCount - 1 Step 1
                        strValue(0) = objPulicParameters.getObjectValue(.Item(i).Item(Josco.JSOA.Common.Data.FlowData.FIELD_GW_B_DUBAN_DBR), "")
                        strValue(1) = objPulicParameters.getObjectValue(.Item(i).Item(Josco.JSOA.Common.Data.FlowData.FIELD_GW_B_DUBAN_BDBR), "")
                        strValue(2) = Format(.Item(i).Item(Josco.JSOA.Common.Data.FlowData.FIELD_GW_B_DUBAN_DBRQ), "yyyy-MM-dd HH:mm")
                        strValue(3) = objPulicParameters.getObjectValue(.Item(i).Item(Josco.JSOA.Common.Data.FlowData.FIELD_GW_B_DUBAN_DBYQ), "")
                        strValue(4) = objPulicParameters.getObjectValue(.Item(i).Item(Josco.JSOA.Common.Data.FlowData.FIELD_GW_B_DUBAN_DBJG), "")
                        If strValue(4) = "" Then
                            strValue(4) = "    " + "没有进行回应！"
                        Else
                            strValue(4) = "    " + strValue(4).Replace(vbCr, vbCr + "    ")
                        End If
                        If strDBSJ = "" Then
                            strDBSJ = "[" + strValue(2) + "][" + strValue(0) + "]向[" + strValue(1) + "]进行督办：" + strValue(3) + vbCr + "回应如下：" + vbCr + strValue(4)
                        Else
                            strDBSJ = strDBSJ + vbCr + "[" + strValue(2) + "][" + strValue(0) + "]向[" + strValue(1) + "]进行督办：" + strValue(3) + vbCr + "回应如下：" + vbCr + strValue(4)
                        End If
                    Next
                End With
                Me.textareaQBJG.InnerText = strDBSJ
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function showModuleData_DBSJ(ByRef strErrMsg As String) As Boolean

            showModuleData_DBSJ = False
        ...
        Private Function initializeControls(ByRef strErrMsg As String) As Boolean
            ...
            Try
                ...
                If Me.IsPostBack = False Then
                    ...
                    '先显示grdDBSJ
                    If Me.showModuleData_DBSJ(strErrMsg) = False Then
                        GoTo errProc
                    End If
                End If
...
errProc:
        ...
        Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            Try
                ...
                '获取附件数据
                If Me.getModuleData_DBSJ(strErrMsg) = False Then
                    GoTo errProc
                End If
                ...
                If Me.IsPostBack = False Then
                    If Me.m_blnSaveScence = False Then
                        If Me.m_objsystemFlowObject.doWriteUserLog(strErrMsg, MyBase.UserId, MyBase.UserPassword, Request.UserHostAddress, Request.UserHostName, "访问了文件[" + Me.m_objsystemFlowObject.FlowData.WJBS + "]的[督办情况]！") = False Then
                            '忽略
                        End If
                    End If
                End If
