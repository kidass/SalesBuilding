        ...
        Private Sub doModify(ByVal strControlId As String)
            ...
            Try
                ...
                If Me.rblSFBZ.SelectedIndex < 0 Then
                    strErrMsg = "错误：没有指定收、付！"
                    GoTo errProc
                End If
                If Me.rblSFDX.SelectedIndex < 0 Then
                    strErrMsg = "错误：没有指定甲、乙！"
                    GoTo errProc
                End If
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    objMessageProcess.doConfirmMessage(Me.popMessageObject, "警告：您确认本款项经过指定的改动后可以审定吗（是/否）？", strControlId, intStep)
                    Exit Try
                Else
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    '记录审计日志
                    Josco.JsKernal.SystemFramework.ApplicationLog.WriteInfo(Request.UserHostAddress, Request.UserHostName, "[" + MyBase.UserId + "]在[财务_实收实付]中对[" + strWYBS + "]执行了[审定]！")
...
errProc:
        ...
        Private Sub doOK(ByVal strControlId As String)
            ...
            Try
                ...
                intStep = 2
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    objMessageProcess.doConfirmMessage(Me.popMessageObject, "警告：您确认本款项可以审定吗（是/否）？", strControlId, intStep)
                    Exit Try
                Else
                    objMessageProcess.doResetPopMessage(Me.popMessageObject)
                End If
...
errProc:
        ...
        Private Sub doReject(ByVal strControlId As String)
            ...
            Try
                ...
                intStep = 2
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    objMessageProcess.doConfirmMessage(Me.popMessageObject, "警告：您确认本款项审核不通过吗（是/否）？", strControlId, intStep)
                    Exit Try
                Else
                    objMessageProcess.doResetPopMessage(Me.popMessageObject)
                End If
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    '记录审计日志
                    Josco.JsKernal.SystemFramework.ApplicationLog.WriteInfo(Request.UserHostAddress, Request.UserHostName, "[" + MyBase.UserId + "]在[财务_实收实付]中对[" + strWYBS + "]执行了[不通过审核]！")
        ...
        Private Sub doPLSDKX(ByVal strControlId As String)
            ...
            Try
                ...
                If intSelected < 1 Then
                    strErrMsg = "错误：没有选中任何款项(选中请在第1列的方框中打钩)！"
                    GoTo errProc
                End If
