                    ...
                    If Not (objTextBox Is Nothing) Then
                        If blnCheck = True Then
                            ...
                            If dblValue1 > dblValue2 Then
                                strErrMsg = "错误：不能超过[" + dblValue2.ToString("#,##0.00") + "]！"
                                GoTo errProc
                            End If
                        End If
        ...
        Private Sub doJsbbsj(ByVal strControlId As String)
            ...
            Try
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    End If
                    If blnHas = True Then
                        objMessageProcess.doConfirmMessage(Me.popMessageObject, "提示：[" + intND.ToString + "]年度[" + intYD.ToString + "]月度的补发数据已经确认，要重新生成吗（确定/取消）？", strControlId, intStep, True)
                        Exit Try
                    Else
                        blnOK = False
                    End If
...
errProc:
        ...
        Private Sub doExport(ByVal strControlId As String)
            ...
            Try
                ...
                Dim strSrcExcelSpec As String = Request.ApplicationPath + Me.m_cstrUrlBaseToExcelMB + "广州兴业_二手报表_中介部二手计佣清单(公佣补发).xls"
...
errProc:
        ...
        Private Sub doGybfqr(ByVal strControlId As String)
            ...
            Try
                ...
                End If
                If blnHas = True Then
                    strErrMsg = "错误：[" + intND.ToString + "]年[" + intYD.ToString + "]月以后存在补发数据，不能确认！"
                    GoTo errProc
                End If
                ...
                '询问
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    objMessageProcess.doConfirmMessage(Me.popMessageObject, "警告：您确定[" + intND.ToString + "]年[" + intYD.ToString + "]月的补发数据正确吗（确定/取消）？", strControlId, intStep)
                    Exit Try
                Else
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    If objsystemEstateErshou.doSaveData_ES_JYHTJSSYJBF(strErrMsg, MyBase.UserId, MyBase.UserPassword, intND, intYD, Me.m_objDataSet) = False Then
                        GoTo errProc
                    End If
                    objMessageProcess.doAlertMessage(Me.popMessageObject, "提示：确认成功！")
                End If
            Catch ex As Exception
                strErrMsg = ex.Message
                GoTo errProc
