...
Namespace Josco.JSOA.web
    ...
    '   　中介部二手区域业绩排行榜处理模块
    ' 更改记录：
    '     zengxianglin 2009-05-21 更改(增加[实结代理费]指标处理)
    '     zengxianglin 2009-06-15 更改(增加[结算]类型处理)
    '----------------------------------------------------------------

    Partial Class estate_es_bb_esphbqyyj
...
#End Region
        ...
        '----------------------------------------------------------------
        Private m_objSaveScence As Josco.JSOA.BusinessFacade.IMEstateEsBbEsphbqyyj
        Private m_blnSaveScence As Boolean

        '----------------------------------------------------------------
        '模块接口参数
        '----------------------------------------------------------------
        Private m_objInterface As Josco.JSOA.BusinessFacade.IEstateEsBbEsphbqyyj
        ...
        Private Function getPrevilegeParams( _
            ...
            Try
                ...
                '是否继续执行
                If Me.m_blnPrevilegeParams(0) = True And Me.m_blnPrevilegeParams(14) = True Then
                    blnContinueExecute = True
                Else
...
errProc:
        ...
        Private Function saveModuleInformation() As String
            ...
            Try
                ...
                '创建对象
                Me.m_objSaveScence = New Josco.JSOA.BusinessFacade.IMEstateEsBbEsphbqyyj
        ...
        Private Function getInterfaceParameters(ByRef strErrMsg As String) As Boolean
            ...
            Try
                ...
                Try
                    ...
                    m_objInterface = CType(objTemp, Josco.JSOA.BusinessFacade.IEstateEsBbEsphbqyyj)
                Catch ex As Exception
                ...
                If Me.IsPostBack = False Then
                    ...
                    Try
                        Me.m_objSaveScence = CType(Session.Item(strSessionId), Josco.JSOA.BusinessFacade.IMEstateEsBbEsphbqyyj)
                    Catch ex As Exception
                        Me.m_objSaveScence = Nothing
                    End Try
                    If Me.m_objSaveScence Is Nothing Then
...
errProc:
        ...
        Private Function getReportParam( _
            ...
            ByRef intType As Integer) As Boolean
        ...
        Private Function getModuleData( _
            ...
            Try
                ...
                Else
                    ...
                    If Me.IsPostBack = False Then
                        '不计算数据
                        Me.m_objDataSet = New Josco.JSOA.Common.Data.estateErshouData(Josco.JSOA.Common.Data.estateErshouData.enumTableType.DC_VT_ES_BB_PHBQYYJ)
                        Dim objDataTable As System.Data.DataTable = Nothing
                        Dim intCount As Integer = 5
                        Dim i As Integer
                        For i = 0 To intCount - 1 Step 1
                            objDataTable = Me.m_objDataSet.Tables(0).Clone()
                            objDataTable.TableName = Josco.JSOA.Common.Data.estateErshouData.TABLE_DC_VT_ES_BB_PHBQYYJ + "_" + i.ToString()
                            Me.m_objDataSet.Tables.Add(objDataTable)
                        Next
                    Else
                        '获取参数
                        If Me.getReportParam(strErrMsg, strKSRQ, strZZRQ, intType) = False Then
                            GoTo errProc
                        End If
                        '重新检索数据
                        If objsystemEstateErshou.getDataSet_BB_ESPHBQYYJ(strErrMsg, MyBase.UserId, MyBase.UserPassword, strKSRQ, strZZRQ, intType, Me.m_objDataSet) = False Then
                            GoTo errProc
                        End If
                    End If
...
errProc:
        ...
        Private Sub doExport(ByVal strControlId As String)
            ...
            Try
                ...
                Dim strSrcExcelSpec As String = Request.ApplicationPath + Me.m_cstrUrlBaseToExcelMB + "广州兴业_二手报表_中介部二手排行榜区域业绩.xls"
                ...
                With objDataTable.Columns
                    .Add("年度", GetType(System.Int32))
                    .Add("月度", GetType(System.Int32))
                    .Add("统计类型", GetType(System.String))
                End With
                objDataRow = objDataTable.NewRow
                objDataRow.Item("年度") = objZZRQ.Year
                objDataRow.Item("月度") = objZZRQ.Month
                objDataRow.Item("统计类型") = strType
                objDataTable.Rows.Add(objDataRow)
                '输出数据
                If objsystemEstateErshou.doExportToExcel(strErrMsg, objDataTable, strDesExcelSpec, "D_公共") = False Then
                    GoTo errProc
                End If
                If objsystemEstateErshou.doExportToExcel(strErrMsg, Me.m_objDataSet.Tables(0), strDesExcelSpec, "D_面积") = False Then
                    GoTo errProc
                End If
                If objsystemEstateErshou.doExportToExcel(strErrMsg, Me.m_objDataSet.Tables(1), strDesExcelSpec, "D_套数") = False Then
                    GoTo errProc
                End If
                If objsystemEstateErshou.doExportToExcel(strErrMsg, Me.m_objDataSet.Tables(2), strDesExcelSpec, "D_宗数") = False Then
                    GoTo errProc
                End If
                If objsystemEstateErshou.doExportToExcel(strErrMsg, Me.m_objDataSet.Tables(3), strDesExcelSpec, "D_代理费") = False Then
                    GoTo errProc
                End If
                If objsystemEstateErshou.doExportToExcel(strErrMsg, Me.m_objDataSet.Tables(4), strDesExcelSpec, "D_按揭费") = False Then
                    GoTo errProc
                End If
                If objsystemEstateErshou.doExportToExcel(strErrMsg, Me.m_objDataSet.Tables(5), strDesExcelSpec, "D_交易额") = False Then
                    GoTo errProc
                End If
                'zengxianglin 2009-05-21
                If objsystemEstateErshou.doExportToExcel(strErrMsg, Me.m_objDataSet.Tables(6), strDesExcelSpec, "D_实结代理费") = False Then
                    GoTo errProc
                End If
                'zengxianglin 2009-05-21
