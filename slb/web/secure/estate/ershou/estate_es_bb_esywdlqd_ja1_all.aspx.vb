...
Namespace Josco.JSOA.web
    ...
    '----------------------------------------------------------------
    ' 命名空间：Josco.JSOA.web
    ' 类名    ：estate_es_bb_esywdlqd_ja1_all
    ' 
    ' 调用性质：
    '     独立运行
    '
    ' 功能描述： 
    '   　中介部二手业务代理清单(接案)(各类状态)(片区|分行|合同)处理模块
    ' 更改记录：
    '     zengxianglin 2009-05-21 创建
    '----------------------------------------------------------------

    Partial Class estate_es_bb_esywdlqd_ja1_all
...
#End Region
        ...
        '----------------------------------------------------------------
        Private m_objSaveScence As Josco.JSOA.BusinessFacade.IMEstateEsBbEsywdlqdJa1All
        Private m_blnSaveScence As Boolean

        '----------------------------------------------------------------
        '模块接口参数
        '----------------------------------------------------------------
        Private m_objInterface As Josco.JSOA.BusinessFacade.IEstateEsBbEsywdlqdJa1All
        ...
        Private Function getPrevilegeParams( _
            ...
            Try
                ...
                '是否继续执行
                If Me.m_blnPrevilegeParams(0) = True And Me.m_blnPrevilegeParams(21) = True Then
                    blnContinueExecute = True
                Else
...
errProc:
        ...
        Private Function saveModuleInformation() As String
            ...
            Try
                ...
                '创建对象
                Me.m_objSaveScence = New Josco.JSOA.BusinessFacade.IMEstateEsBbEsywdlqdJa1All
        ...
        Private Function getInterfaceParameters(ByRef strErrMsg As String) As Boolean
            ...
            Try
                ...
                Try
                    ...
                    m_objInterface = CType(objTemp, Josco.JSOA.BusinessFacade.IEstateEsBbEsywdlqdJa1All)
                Catch ex As Exception
                ...
                If Me.IsPostBack = False Then
                    ...
                    Try
                        Me.m_objSaveScence = CType(Session.Item(strSessionId), Josco.JSOA.BusinessFacade.IMEstateEsBbEsywdlqdJa1All)
                    Catch ex As Exception
                        Me.m_objSaveScence = Nothing
                    End Try
                    If Me.m_objSaveScence Is Nothing Then
...
errProc:
        ...
        Private Function getQueryString( _
            ...
            Try
                '按[显示级别]搜索
                Dim strXSJB As String = Josco.JSOA.Common.Data.estateErshouData.FIELD_DC_VT_ES_BB_ESYWDLQD_JA1_ALL_XSJB
...
errProc:
        ...
        Private Function getModuleData( _
            ...
            Dim strTable As String = Josco.JSOA.Common.Data.estateErshouData.TABLE_DC_VT_ES_BB_ESYWDLQD_JA1_ALL
            ...
            Try
                ...
                Else
                    ...
                    If Me.IsPostBack = False Then
                        '不计算数据
                        Me.m_objDataSet = New Josco.JSOA.Common.Data.estateErshouData(Josco.JSOA.Common.Data.estateErshouData.enumTableType.DC_VT_ES_BB_ESYWDLQD_JA1_ALL)
                    Else
                        '获取参数
                        ...
                        End If
                        '重新检索数据
                        If objsystemEstateErshou.getDataSet_BB_ESYWDLQD_JA1_ALL(strErrMsg, MyBase.UserId, MyBase.UserPassword, strKSRQ, strZZRQ, strBMDM, Me.m_objDataSet) = False Then
                            GoTo errProc
                        End If
                    End If
        ...
        Sub grdContent_ItemDataBound(ByVal sender As Object, ByVal e As DataGridItemEventArgs) Handles grdContent.ItemDataBound
            ...
            Try
                ...
                Else
                    ...
                    intFZJB = objDataGridProcess.getDataGridColumnIndex(Me.grdContent, Josco.JSOA.Common.Data.estateErshouData.FIELD_DC_VT_ES_BB_ESYWDLQD_JA1_ALL_XSJB)
        ...
        Private Sub doExport(ByVal strControlId As String)
            ...
            Try
                ...
                If Me.getReportParam(strErrMsg, strKSRQ, strZZRQ, strBMDM) = False Then
                    GoTo errProc
                End If
                Dim strFName As String = Josco.JSOA.Common.Data.estateErshouData.FIELD_DC_VT_ES_BB_ESYWDLQD_JA1_ALL_XSJB
                ...
                Dim strSrcExcelSpec As String = Request.ApplicationPath + Me.m_cstrUrlBaseToExcelMB + "广州兴业_二手报表_中介部二手业务代理清单(接案1)(各类状态).xls"
