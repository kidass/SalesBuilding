...
Namespace Josco.JSOA.web
    ...
    '
    ' 接口参数：
    '     参见接口类IFlowImportZS描述
    '----------------------------------------------------------------

    Public Class flow_importzs
...
#Region " Web 窗体设计器生成的代码 "
        ...
        Protected WithEvents htxtFileUpload As System.Web.UI.HtmlControls.HtmlInputHidden
...
#End Region
        ...
        '----------------------------------------------------------------
        Private m_objSaveScence As Josco.JSOA.BusinessFacade.IMFlowImportZS
        Private m_blnSaveScence As Boolean

        '----------------------------------------------------------------
        '模块接口参数
        '----------------------------------------------------------------
        Private m_objInterface As Josco.JSOA.BusinessFacade.IFlowImportZS
        ...
        Private Sub restoreModuleInformation(ByVal strSessionId As String)
            ...
            Try
                ...
                With Me.m_objSaveScence
                    ...
                    Me.htxtFileUpload.Value = .htxtFileUpload
        ...
        Private Function saveModuleInformation() As String
            ...
            Try
                ...
                '创建对象
                Me.m_objSaveScence = New Josco.JSOA.BusinessFacade.IMFlowImportZS
                ...
                With Me.m_objSaveScence
                    ...
                    .htxtFileUpload = Me.htxtFileUpload.Value
        ...
        Private Function getInterfaceParameters(ByRef strErrMsg As String, ByRef blnContinue As Boolean) As Boolean
            ...
            Try
                ...
                Try
                    ...
                    m_objInterface = CType(objTemp, Josco.JSOA.BusinessFacade.IFlowImportZS)
                Catch ex As Exception
                ...
                If Me.IsPostBack = False Then
                    ...
                    Try
                        Me.m_objSaveScence = CType(Session.Item(strSessionId), Josco.JSOA.BusinessFacade.IMFlowImportZS)
                    Catch ex As Exception
                        Me.m_objSaveScence = Nothing
                    End Try
...
errProc:
        ...
        Private Function showModuleData_Main(ByRef strErrMsg As String) As Boolean
            ...
            Try
                Me.lblTitle.Text = Me.m_objInterface.iTitle
...
errProc:
        ...
        Private Sub doDeleteTempFiles()
            ...
            Try
                '本次操作形成的缓存文件
                Dim strOldFile As String = Me.htxtFileUpload.Value.Trim
                If strOldFile <> "" Then
                    If objBaseLocalFile.doDeleteFile(strErrMsg, strOldFile) = False Then
                        '形成垃圾文件
                    End If
                    Me.htxtFileUpload.Value = ""
                End If
            Catch ex As Exception
            End Try
        ...
        Private Function doUploadFile(ByRef strErrMsg As String) As Boolean
            ...
            Try
                ...
                If Not (Me.fileUpload.PostedFile Is Nothing) Then
                    If Me.fileUpload.PostedFile.FileName <> "" Then
                        '获取新文件路径
                        If objBaseLocalFile.doCreateTempFile(strErrMsg, Me.fileUpload.PostedFile.FileName, True, strFileName) = False Then
                            GoTo errProc
                        End If
                        If strFileName = "" Then
                        ...
                        '保存WEB本地文件路径
                        Me.htxtFileUpload.Value = strFileSpec
                    End If
                End If
            Catch ex As Exception
                strErrMsg = ex.Message
        ...
        Private Sub doOK(ByVal strControlId As String)
            ...
            Try
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    '检查
                    If Me.htxtFileUpload.Value.Trim = "" Then
                        strErrMsg = "错误：没有指定要导入的文件！"
                        GoTo errProc
                    End If

                    '询问
                    Dim strZSWJ As String
                    If Me.m_objsystemFlowObject.getZSWJ(strErrMsg, strZSWJ) = False Then
                        GoTo errProc
                    End If
                    If strZSWJ <> "" Then
                        objMessageProcess.doConfirmMessage(Me.popMessageObject, "提示：正式文件已经导入，要重新导入吗（是/否）？", strControlId, intStep)
                        Exit Try
                    End If
                Else
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    '导入正式文件
                    If Me.m_objsystemFlowObject.doImportZS(strErrMsg, Me.htxtFileUpload.Value) = False Then
                        GoTo errProc
                    End If

                    '记录访问审计日志
                    If Me.m_objsystemFlowObject.doWriteUserLog(strErrMsg, MyBase.UserId, MyBase.UserPassword, Request.UserHostAddress, Request.UserHostName, "导入了文件[" + Me.m_objsystemFlowObject.FlowData.WJBS + "]的[正式文件]！") = False Then
                        '忽略
                    End If
