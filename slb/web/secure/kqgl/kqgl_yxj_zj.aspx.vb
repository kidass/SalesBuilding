...
#End Region
        ...
        Private Const m_cstrCheckBoxIdInDataGrid_CHKALL As String = "chkALL"
        Private Const m_cstrCheckBoxIdInDataGrid_CHKZJALL As String = "chkZJALL"
        Private Const m_cstrCheckBoxIdInDataGrid_RSZJ As String = "chkKQZJ"
        Private Const m_cstrCheckBoxIdInDataGrid_CHKDEALL As String = "chkDEALL"
        Private Const m_cstrCheckBoxIdInDataGrid_CHKDEZJ As String = "chkDEZJ"
        Private Const m_cstrCheckBoxIdInDataGrid_CHKYXJALL As String = "chkYXJALL"
        Private Const m_cstrCheckBoxIdInDataGrid_CHKDEYXJ As String = "chkDEYXJ"
        '包含网格的DIV对象ID
        ...
        Private m_objDataSet_RSZJ As Josco.JSOA.Common.Data.estateRenshiXingyeData
        Private m_objDataSet_KQZJ As Josco.JSOA.Common.Data.kaoqinguanliData
        Private m_objDataSet_List As Josco.JSOA.Common.Data.kaoqinguanliData
        Private m_objDataSet_YXJ As Josco.JSOA.Common.Data.kaoqinguanliData
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function getModuleData_RSZJ( _
            ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.estateRenshiXingyeData.TABLE_DC_B_RS_ZHIJIDINGYI
            Dim m_objsystemEstateRenshiXingye As New Josco.JSOA.BusinessFacade.systemEstateRenshiXingye

            getModuleData_RSZJ = False

            Try

                '释放资源
                Josco.JSOA.Common.Data.estateRenshiXingyeData.SafeRelease(Me.m_objDataSet_RSZJ)

                '重新检索数据              
                If m_objsystemEstateRenshiXingye.getDataSet_ZhijiDingyi(strErrMsg, MyBase.UserId, MyBase.UserPassword, "", m_objDataSet_RSZJ) = False Then
                    GoTo errProc
                End If
            ...
            Josco.JSOA.BusinessFacade.systemEstateRenshiXingye.SafeRelease(m_objsystemEstateRenshiXingye)

            getModuleData_RSZJ = True
            Exit Function
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function getModuleData_KQZJ( _
            ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.kaoqinguanliData.TABLE_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI
            Dim m_objsystemKaoqinguanli As New Josco.JSOA.BusinessFacade.systemKaoqinguanli

            getModuleData_KQZJ = False

            Try

                '释放资源
                Josco.JSOA.Common.Data.kaoqinguanliData.SafeRelease(Me.m_objDataSet_KQZJ)

                '重新检索数据              
                If m_objsystemKaoqinguanli.getDataSet_KQZJ(strErrMsg, MyBase.UserId, MyBase.UserPassword, m_objDataSet_KQZJ) = False Then
                    GoTo errProc
                End If
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function getModuleData_YXJ( _
            ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.kaoqinguanliData.TABLE_KQ_VT_YUEYINGXIUJIA
            Dim m_objsystemKaoqinguanli As New Josco.JSOA.BusinessFacade.systemKaoqinguanli

            getModuleData_YXJ = False

            Try

                '释放资源
                Josco.JSOA.Common.Data.kaoqinguanliData.SafeRelease(Me.m_objDataSet_YXJ)

                '重新检索数据              
                If m_objsystemKaoqinguanli.getDataYXJBZ(strErrMsg, MyBase.UserId, MyBase.UserPassword, m_objDataSet_YXJ) = False Then
                    GoTo errProc
                End If
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function searchModuleData_RSZJ(ByRef strErrMsg As String) As Boolean

            searchModuleData_RSZJ = False

            Try

                '搜索数据
                If Me.getModuleData_RSZJ(strErrMsg) = False Then
                    GoTo errProc
                End If
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function searchModuleData_KQZJ(ByRef strErrMsg As String) As Boolean

            searchModuleData_KQZJ = False

            Try

                '搜索数据
                If Me.getModuleData_KQZJ(strErrMsg) = False Then
                    GoTo errProc
                End If
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function searchModuleData_YXJ(ByRef strErrMsg As String) As Boolean

            searchModuleData_YXJ = False

            Try

                '搜索数据
                If Me.getModuleData_YXJ(strErrMsg) = False Then
                    GoTo errProc
                End If
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function showDataGridInfo_RSZJ(ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.estateRenshiXingyeData.TABLE_DC_B_RS_ZHIJIDINGYI
            Dim objPulicParameters As New Josco.JsKernal.Common.Utilities.PulicParameters
            Dim objDataGridProcess As New Josco.JsKernal.web.DataGridProcess

            showDataGridInfo_RSZJ = False
            ...
            Try
                '在获取数据时已经恢复了RowFilter、Sort的现场
                '设置数据源
                If Me.m_objDataSet_RSZJ Is Nothing Then
                    Me.grdKQZJ.DataSource = Nothing
                Else
                    With Me.m_objDataSet_RSZJ.Tables(strTable)
                        Me.grdKQZJ.DataSource = .DefaultView
                    End With
                End If

                '调整网格参数
                With Me.m_objDataSet_RSZJ.Tables(strTable)
                    If objDataGridProcess.onBeforeDataGridBind(strErrMsg, Me.grdKQZJ, .DefaultView.Count) = False Then
                        GoTo errProc
                    End If
                End With
                ...
                ''恢复列标题中的排序信息
                'If intSortColumnIndex >= 0 Then
                '    objDataGridProcess.doClearSortCharInDataGridHead(Me.grdKQZJ)
                '    With Me.grdKQZJ.Columns(intSortColumnIndex)
                '        .HeaderText = objDataGridProcess.getColumnSortHeadString(.HeaderText, objSortType)
                '    End With
                'End If
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function showDataGridInfo_KQZJ(ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.kaoqinguanliData.TABLE_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI
            Dim objPulicParameters As New Josco.JsKernal.Common.Utilities.PulicParameters
            Dim objDataGridProcess As New Josco.JsKernal.web.DataGridProcess

            showDataGridInfo_KQZJ = False
            ...
            Try
                '在获取数据时已经恢复了RowFilter、Sort的现场
                '设置数据源
                If Me.m_objDataSet_KQZJ Is Nothing Then
                    Me.grdZJKQ.DataSource = Nothing
                Else
                    With Me.m_objDataSet_KQZJ.Tables(strTable)
                        Me.grdZJKQ.DataSource = .DefaultView
                    End With
                End If

                '调整网格参数
                With Me.m_objDataSet_KQZJ.Tables(strTable)
                    If objDataGridProcess.onBeforeDataGridBind(strErrMsg, Me.grdZJKQ, .DefaultView.Count) = False Then
                        GoTo errProc
                    End If
                End With
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function showDataGridInfo_YXJ(ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.kaoqinguanliData.TABLE_KQ_VT_YUEYINGXIUJIA
            Dim objPulicParameters As New Josco.JsKernal.Common.Utilities.PulicParameters
            Dim objDataGridProcess As New Josco.JsKernal.web.DataGridProcess

            showDataGridInfo_YXJ = False
            ...
            Try
                '在获取数据时已经恢复了RowFilter、Sort的现场
                '设置数据源
                If Me.m_objDataSet_YXJ Is Nothing Then
                    Me.grdYXJ.DataSource = Nothing
                Else
                    With Me.m_objDataSet_YXJ.Tables(strTable)
                        Me.grdYXJ.DataSource = .DefaultView
                    End With
                End If

                '调整网格参数
                With Me.m_objDataSet_YXJ.Tables(strTable)
                    If objDataGridProcess.onBeforeDataGridBind(strErrMsg, Me.grdYXJ, .DefaultView.Count) = False Then
                        GoTo errProc
                    End If
                End With
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function showModuleData_RSZJ(ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.estateRenshiXingyeData.TABLE_DC_B_RS_ZHIJIDINGYI
            Dim objDataGridProcess As New Josco.JsKernal.web.DataGridProcess

            showModuleData_RSZJ = False

            Try
                '显示网格信息
                If Me.showDataGridInfo_RSZJ(strErrMsg) = False Then
                    GoTo errProc
                End If

                '显示与网格紧密相关的操作或信息提示
                With Me.m_objDataSet_RSZJ.Tables(strTable).DefaultView
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function showModuleData_KQZJ(ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.kaoqinguanliData.TABLE_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI
            Dim objDataGridProcess As New Josco.JsKernal.web.DataGridProcess

            showModuleData_KQZJ = False

            Try
                '显示网格信息
                If Me.showDataGridInfo_KQZJ(strErrMsg) = False Then
                    GoTo errProc
                End If

                '显示与网格紧密相关的操作或信息提示
                With Me.m_objDataSet_KQZJ.Tables(strTable).DefaultView
...
errProc:
        ...
        '     False          ：失败
        '----------------------------------------------------------------
        Private Function showModuleData_YXJ(ByRef strErrMsg As String) As Boolean

            Dim strTable As String = Josco.JSOA.Common.Data.kaoqinguanliData.TABLE_KQ_VT_YUEYINGXIUJIA
            Dim objDataGridProcess As New Josco.JsKernal.web.DataGridProcess

            showModuleData_YXJ = False

            Try
                '显示网格信息
                If Me.showDataGridInfo_YXJ(strErrMsg) = False Then
                    GoTo errProc
                End If

                '显示与网格紧密相关的操作或信息提示
                With Me.m_objDataSet_YXJ.Tables(strTable).DefaultView
        ...
        Private Function initializeControls(ByRef strErrMsg As String) As Boolean
            ...
            If Me.IsPostBack = False Then
                ...
                If Me.m_blnSaveScence = False Then
                    ...
                    If Me.searchModuleData_RSZJ(strErrMsg) = False Then
                        GoTo errProc
                    End If

                    If Me.searchModuleData_KQZJ(strErrMsg) = False Then
                        GoTo errProc
                    End If

                    If Me.searchModuleData_YXJ(strErrMsg) = False Then
                        GoTo errProc
                    End If
                Else
                ...
                If Me.showModuleData_RSZJ(strErrMsg) = False Then
                    GoTo errProc
                End If


                If Me.showModuleData_KQZJ(strErrMsg) = False Then
                    GoTo errProc
                End If

                If Me.showModuleData_YXJ(strErrMsg) = False Then
                    GoTo errProc
                End If
            End If
...
errProc:
        ...
        Private Sub doSelectAll_RSZJ(ByVal strControlId As String)
            ...
            Try
                If objDataGridProcess.doCheckedDataGridCheckBox(strErrMsg, Me.grdKQZJ, 0, Me.m_cstrCheckBoxIdInDataGrid_RSZJ, True) = False Then
                    GoTo errProc
                End If
            Catch ex As Exception
                strErrMsg = ex.Message
...
errProc:
        ...
        Private Sub doDeSelectAll_RSZJ(ByVal strControlId As String)
            ...
            Try
                If objDataGridProcess.doCheckedDataGridCheckBox(strErrMsg, Me.grdKQZJ, 0, Me.m_cstrCheckBoxIdInDataGrid_RSZJ, False) = False Then
                    GoTo errProc
                End If
            Catch ex As Exception
                strErrMsg = ex.Message
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    For i = intRows - 1 To 0 Step -1
                        If objDataGridProcess.isDataGridItemChecked(Me.grdKQJLG.Items(i), 0, Me.m_cstrCheckBoxIdInDataGrid_RSLYGL) = True Then
                            ...
                            objNewData.Add(Josco.JSOA.Common.Data.kaoqinguanliData.FIELD_KQ_B_YUEYINGXIUJIA_TS, Me.txtYF.Text.Trim)
...
errProc:
        ...
        Private Function doUpdate(ByRef strErrMsg As String, ByVal strControlId As String) As Boolean
            ...
            Try
                intStep = 1
                '检查选择
                Dim intSelect_bm As Integer = 0
                Dim intSelect_zj As Integer = 0
                Dim intRows_bm As Integer
                Dim intRows_zj As Integer
                Dim i As Integer
                Dim j As Integer

                intRows_bm = Me.grdKQJLG.Items.Count
                intRows_zj = Me.grdKQZJ.Items.Count
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    For i = 0 To intRows_bm - 1 Step 1
                        If objDataGridProcess.isDataGridItemChecked(Me.grdKQJLG.Items(i), 0, Me.m_cstrCheckBoxIdInDataGrid_RSLYGL) = True Then
                            intSelect_bm += 1
                        End If
                    Next
                    If intSelect_bm < 1 Then
                        strErrMsg = "错误：未选择要更新的单位！"
                        GoTo errProc
                    End If

                    For j = 0 To intRows_zj - 1 Step 1
                        If objDataGridProcess.isDataGridItemChecked(Me.grdKQZJ.Items(j), 0, Me.m_cstrCheckBoxIdInDataGrid_RSZJ) = True Then
                            intSelect_zj += 1
                        End If
                    Next
                    If intSelect_zj < 1 Then
                        strErrMsg = "错误：未选择要更新的职级！"
                        GoTo errProc
                    End If
                End If
                ...
                intStep = 2
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    objMessageProcess.doConfirmMessage(Me.popMessageObject, "提示：您确实准备更新选定的[" + intSelect_bm.ToString() + "]个的单位的[" + intSelect_zj.ToString() + "]个职级吗（是/否）？", strControlId, intStep)
                    Exit Try
                Else
                    objMessageProcess.doResetPopMessage(Me.popMessageObject)
                End If
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    For i = intRows_bm - 1 To 0 Step -1
                        If objDataGridProcess.isDataGridItemChecked(Me.grdKQJLG.Items(i), 0, Me.m_cstrCheckBoxIdInDataGrid_RSLYGL) = True Then
                            intPos = objDataGridProcess.getRecordPosition(i, Me.grdKQJLG.CurrentPageIndex, Me.grdKQJLG.PageSize)
                            ...
                            For j = intRows_zj - 1 To 0 Step -1
                                If objDataGridProcess.isDataGridItemChecked(Me.grdKQZJ.Items(j), 0, Me.m_cstrCheckBoxIdInDataGrid_RSZJ) = True Then
                                    objNewData.Clear()
                                    intIndex = objDataGridProcess.getDataGridColumnIndex(Me.grdKQZJ, Josco.JSOA.Common.Data.estateRenshiXingyeData.FIELD_DC_B_RS_ZHIJIDINGYI_ZJDM)
                                    strZJDM = objDataGridProcess.getDataGridCellValue(Me.grdKQZJ.Items(j), intIndex)

                                    objNewData.Add(Josco.JSOA.Common.Data.kaoqinguanliData.FIELD_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI_ZZDM, strZZDM)
                                    objNewData.Add(Josco.JSOA.Common.Data.kaoqinguanliData.FIELD_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI_BZXL, Me.rblRYLX.SelectedValue)
                                    objNewData.Add(Josco.JSOA.Common.Data.kaoqinguanliData.FIELD_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI_ZJDM, strZJDM)
                                    objNewData.Add(Josco.JSOA.Common.Data.kaoqinguanliData.FIELD_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI_KSSJ, Me.txtJQ.Text.Trim)

                                    '相应处理
                                    If objsystemKaoqinguanli.doUpdate_YXJ_ZJ( _
                                        strErrMsg, MyBase.UserId, MyBase.UserPassword, _
                                         objNewData) = False Then
                                        GoTo errProc
                                    End If
                                    ...
                                    '记录日志
                                    Josco.JsKernal.SystemFramework.ApplicationLog.WriteInfo(Request.UserHostAddress, Request.UserHostName, "[" + MyBase.UserId + "]对单位[" + strZZDM + "]更新了[" + strZJDM + "]的月应休假！")
...
errProc:
        ...
        Private Function doDelete(ByRef strErrMsg As String, ByVal strControlId As String) As Boolean
            ...
            Try
                ...
                intRows_zj = Me.grdZJKQ.Items.Count
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    
                    For j = 0 To intRows_zj - 1 Step 1
                        If objDataGridProcess.isDataGridItemChecked(Me.grdZJKQ.Items(j), 0, Me.m_cstrCheckBoxIdInDataGrid_CHKDEZJ) = True Then
                            intSelect_zj += 1
                        End If
                    Next
                    If intSelect_zj < 1 Then
                        strErrMsg = "错误：未选择要删除的职级！"
                        GoTo errProc
                    End If
                End If
                ...
                intStep = 2
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    objMessageProcess.doConfirmMessage(Me.popMessageObject, "提示：您确实准备删除选定的[" + intSelect_zj.ToString() + "]个职级标准吗（是/否）？", strControlId, intStep)
                    Exit Try
                Else
                    objMessageProcess.doResetPopMessage(Me.popMessageObject)
                End If
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    For i = intRows_zj - 1 To 0 Step -1
                        If objDataGridProcess.isDataGridItemChecked(Me.grdZJKQ.Items(i), 0, Me.m_cstrCheckBoxIdInDataGrid_CHKDEZJ) = True Then
                            intPos = objDataGridProcess.getRecordPosition(i, Me.grdZJKQ.CurrentPageIndex, Me.grdZJKQ.PageSize)

                            intIndex = objDataGridProcess.getDataGridColumnIndex(Me.grdZJKQ, Josco.JSOA.Common.Data.kaoqinguanliData.FIELD_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI_BZBS)
                            strBZBS = objDataGridProcess.getDataGridCellValue(Me.grdZJKQ.Items(i), intIndex)

                            intIndex = objDataGridProcess.getDataGridColumnIndex(Me.grdZJKQ, Josco.JSOA.Common.Data.kaoqinguanliData.FIELD_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI_ZZDM)
                            strZZDM = objDataGridProcess.getDataGridCellValue(Me.grdZJKQ.Items(i), intIndex)

                            intIndex = objDataGridProcess.getDataGridColumnIndex(Me.grdZJKQ, Josco.JSOA.Common.Data.kaoqinguanliData.FIELD_KQ_B_XIUJIA_BIAOZHUNBIANHUA_ZHIJI_ZJDM)
                            strZJDM = objDataGridProcess.getDataGridCellValue(Me.grdZJKQ.Items(i), intIndex)

                            '相应处理
                            If objsystemKaoqinguanli.doDelete_YXJ_ZJ( _
                                strErrMsg, MyBase.UserId, MyBase.UserPassword, _
                                 strBZBS) = False Then
                                GoTo errProc
                            End If
                            ...
                            '记录日志
                            Josco.JsKernal.SystemFramework.ApplicationLog.WriteInfo(Request.UserHostAddress, Request.UserHostName, "[" + MyBase.UserId + "]对单位[" + strZZDM + "]删除了[" + strZJDM + "]的月应休假！")
                        End If
                    Next
...
errProc:
        ...
        Public Sub chkALL_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)

            Dim objMessageProcess As New Josco.JsKernal.web.MessageProcess
            Dim strErrMsg As String
            Dim chkAll As System.Web.UI.WebControls.CheckBox
            Dim objDataGridItem As DataGridItem


            Try
                objDataGridItem = CType(CType(sender, System.Web.UI.WebControls.CheckBox).Parent.Parent, DataGridItem)
                chkAll = CType(objDataGridItem.FindControl(Me.m_cstrCheckBoxIdInDataGrid_CHKALL), System.Web.UI.WebControls.CheckBox)
                If chkAll.Checked Then
                    Me.doSelectAll_RSLYGL("chkall")
                Else
                    Me.doDeSelectAll_RSLYGL("chkALL")
                End If
            Catch ex As Exception
                strErrMsg = ex.Message
                GoTo errProc
...
errProc:
        ...
        Public Sub chkZJALL_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)

            Dim objMessageProcess As New Josco.JsKernal.web.MessageProcess
            Dim strErrMsg As String
            Dim chkZJALL As System.Web.UI.WebControls.CheckBox
            Dim objDataGridItem As DataGridItem


            Try
                objDataGridItem = CType(CType(sender, System.Web.UI.WebControls.CheckBox).Parent.Parent, DataGridItem)
                chkZJALL = CType(objDataGridItem.FindControl(Me.m_cstrCheckBoxIdInDataGrid_CHKZJALL), System.Web.UI.WebControls.CheckBox)
                If chkZJALL.Checked Then
                    Me.doSelectAll_RSZJ("chkZJALL")
                Else
                    Me.doDeSelectAll_RSZJ("chkZJALL")
                End If
            Catch ex As Exception
                strErrMsg = ex.Message
                GoTo errProc
...
errProc:
        ...
        Public Sub chkDEALL_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)

            Dim objMessageProcess As New Josco.JsKernal.web.MessageProcess
            Dim strErrMsg As String
            Dim chkDEALL As System.Web.UI.WebControls.CheckBox
            Dim objDataGridItem As DataGridItem
            Dim objDataGridProcess As New Josco.JsKernal.web.DataGridProcess

            Try
                objDataGridItem = CType(CType(sender, System.Web.UI.WebControls.CheckBox).Parent.Parent, DataGridItem)
                chkDEALL = CType(objDataGridItem.FindControl(Me.m_cstrCheckBoxIdInDataGrid_CHKDEALL), System.Web.UI.WebControls.CheckBox)
                If chkDEALL.Checked Then
                    If objDataGridProcess.doCheckedDataGridCheckBox(strErrMsg, Me.grdZJKQ, 0, Me.m_cstrCheckBoxIdInDataGrid_CHKDEZJ, True) = False Then
                        GoTo errProc
                    End If
                Else
                    If objDataGridProcess.doCheckedDataGridCheckBox(strErrMsg, Me.grdZJKQ, 0, Me.m_cstrCheckBoxIdInDataGrid_CHKDEZJ, False) = False Then
                        GoTo errProc
                    End If
                End If
            Catch ex As Exception
...
errProc:
        ...
        Public Sub chkYXJALL_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)

            Dim objMessageProcess As New Josco.JsKernal.web.MessageProcess
            Dim strErrMsg As String
            Dim chkYXJALL As System.Web.UI.WebControls.CheckBox
            Dim objDataGridItem As DataGridItem
            Dim objDataGridProcess As New Josco.JsKernal.web.DataGridProcess

            Try
                objDataGridItem = CType(CType(sender, System.Web.UI.WebControls.CheckBox).Parent.Parent, DataGridItem)
                chkYXJALL = CType(objDataGridItem.FindControl(Me.m_cstrCheckBoxIdInDataGrid_CHKYXJALL), System.Web.UI.WebControls.CheckBox)
                If chkYXJALL.Checked Then
                    If objDataGridProcess.doCheckedDataGridCheckBox(strErrMsg, Me.grdZJKQ, 0, Me.m_cstrCheckBoxIdInDataGrid_CHKDEYXJ, True) = False Then
                        GoTo errProc
                    End If
                Else
                    If objDataGridProcess.doCheckedDataGridCheckBox(strErrMsg, Me.grdZJKQ, 0, Me.m_cstrCheckBoxIdInDataGrid_CHKDEYXJ, False) = False Then
                        GoTo errProc
                    End If
                End If
            Catch ex As Exception
