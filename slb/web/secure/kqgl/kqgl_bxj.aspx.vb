...
errProc:
        ...
        Private Function getModuleData_RSLYGL( _
            ...
            Dim strTable As String = Josco.JSOA.Common.Data.kaoqinguanliData.TABLE_KQ_VT_BUXIUJIA
            ...
            Try
                ...
                Dim strNF As String = ""
                Dim strYF As String = ""
                If m_objsystemKaoqinguanli.getDataBuxiujia(strErrMsg, MyBase.UserId, MyBase.UserPassword, strZZDM, m_objDataSet_RSLYGL) = False Then
                    GoTo errProc
                End If
...
errProc:
        ...
        Private Function showDataGridInfo_RSLYGL(ByRef strErrMsg As String) As Boolean
            ...
            Try
                ...
                Me.grdKQJLG.Columns(2).HeaderText = Month(dtTime.AddMonths(-3)).ToString + "月未休天数"
                Me.grdKQJLG.Columns(3).HeaderText = Month(dtTime.AddMonths(-2)).ToString + "月补休天数"
                Me.grdKQJLG.Columns(4).HeaderText = Month(dtTime.AddMonths(-1)).ToString + "月补休天数"

                Me.grdKQJLG.Columns(5).HeaderText = Month(dtTime.AddMonths(-2)).ToString + "月未休天数"
                Me.grdKQJLG.Columns(6).HeaderText = Month(dtTime.AddMonths(-1)).ToString + "月补休天数"
                Me.grdKQJLG.Columns(7).HeaderText = Month(dtTime).ToString + "月补休天数"

                Me.grdKQJLG.Columns(8).HeaderText = Month(dtTime.AddMonths(-1)).ToString + "月未休天数"
                Me.grdKQJLG.Columns(9).HeaderText = Month(dtTime).ToString + "月补休天数"
                Me.grdKQJLG.Columns(10).HeaderText = Month(dtTime.AddMonths(1)).ToString + "月补休天数"
...
errProc:
        ...
        Private Function doNFList( _
            ...
            Try
                ...
                For i = 0 To 1 Step 1
                    If i = 0 Then
                        strCode = Year(Now).ToString
                        strName = Year(Now).ToString
                    Else
                        strCode = Year(Now.AddYears(1)).ToString
                        strName = Year(Now.AddYears(1)).ToString
                    End If
...
errProc:
        ...
        Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
            ...
            If Me.IsPostBack = False Then
                If Me.doNFList(strErrMsg, Me.ddlNF) = False Then
                    GoTo errProc
                End If
            End If
        ...
        Private Function doSet(ByRef strErrMsg As String, ByVal strControlId As String) As Boolean
            ...
            Try
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    objMessageProcess.doConfirmMessage(Me.popMessageObject, "提示：您确实准备生成[" + strNF + strYF + "]的补休吗（是/否）？", strControlId, intStep)
                    Exit Try
                Else
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    If CInt(strYF) < 10 Then
                        strYF = "0" + strYF
                    End If
                    strKSRQ = strNF + "-" + strYF + "-" + "01"
                    If DateDiff(DateInterval.Month, CDate(strKSRQ), Now) < 1 Then
                        strErrMsg = "错误：生成补休的日期，大于当月！请查证！"
                        GoTo errProc
                    End If
                    strZZDM = Me.htxtBM.Value.Trim

                    strJSRQ = CDate(strKSRQ).AddDays(1 - CDate(strKSRQ).Day).AddMonths(1).AddDays(-1).ToString

                    '相应处理
                    If objsystemKaoqinguanli.doSet_BXJ( _
                        strErrMsg, MyBase.UserId, MyBase.UserPassword, _
                         strKSRQ, strJSRQ, strZZDM) = False Then
                        GoTo errProc
                    End If

                    '记录日志
                    Josco.JsKernal.SystemFramework.ApplicationLog.WriteInfo(Request.UserHostAddress, Request.UserHostName, "[" + MyBase.UserId + "]生成了[" + strNF + "-" + strYF + "]的补休假！")
...
errProc:
        ...
        Public Sub txtBM_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtBM.TextChanged
            If Me.txtBM.Text.Trim = "" Then
                Me.htxtBM.Value = ""
            End If
        End Sub
    End Class
