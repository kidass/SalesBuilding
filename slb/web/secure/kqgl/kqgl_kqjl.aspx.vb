...
Namespace Josco.JSOA.web
    ...
    Partial Public Class kqgl_kqjl
                    ...
                    If objIDmxzZzjg.oExitMode = True Then
                        Select Case objIDmxzZzjg.iSourceControlId.ToUpper
                            ...
                            Case "btnSelect_DRDW".ToUpper
                                Me.txtDR.Text = Replace(objIDmxzZzjg.oBumenList, "售楼部", "")

                            Case "btnSelect_DCDW".ToUpper
                                Me.txtDC.Text = Replace(objIDmxzZzjg.oBumenList, "售楼部", "")
...
errProc:
        ...
        Private Sub doPrint(ByVal strControlId As String)
            ...
            Try
                ...
                '检查模版文件
                Dim strMBURL As String = Request.ApplicationPath + Me.m_cstrExcelMBRelativePathToAppRoot + "考勤表.xls"
                'Dim strMBURL As String = Request.ApplicationPath + Me.m_cstrExcelMBRelativePathToAppRoot + "工资表.xls"
                ...
                If objMessageProcess.isExecuteCode(Request, Me.popMessageObject.UniqueID, intStep) = True Then
                    ...
                    For i = intRows - 1 To 0 Step -1
                        If objDataGridProcess.isDataGridItemChecked(Me.grdKQJLG.Items(i), 0, Me.m_cstrCheckBoxIdInDataGrid_RSLYGL) = True Then
                            ...
                            '检查日期是否过期, SA除外
                            Dim strDate As String
                            If MyBase.UserId <> "sa" Then
                                If m_blnPrevilegeParams(3) = False Then
                                    For k = 0 To intCount_RQ - 1 Step 1
                                        strDate = Mid(strKQRQ(k), 1, 4) + "-" + Mid(strKQRQ(k), 5, 2) + "-" + Mid(strKQRQ(k), 7, 2)
                                        If DateDiff(DateInterval.Month, CDate(strDate), Now) <> 0 Then
                                            strErrMsg = "提示：日期" + strKQRQ(k) + "无效值！"
                                            GoTo errProc
                                        End If
                                    Next
                                End If
                    ...
                    '检查不能在两个以上的部门，在相同的日期也做了同样的考勤
                    Dim blnMutilCheck As Boolean = False
                        ...
                        For i = intRows - 1 To 0 Step -1
                            If objDataGridProcess.isDataGridItemChecked(Me.grdKQJLG.Items(i), 0, Me.m_cstrCheckBoxIdInDataGrid_RSLYGL) = True Then
                                ...
                                For k = 0 To intCount_RQ - 1 Step 1
                                    ...
                                    '相应处理
                                    If objsystemKaoqinguanli.doCheck_Kaoqinjilu( _
                                         strErrMsg, MyBase.UserId, MyBase.UserPassword, blnMutilCheck, _
                                         objNewData, blnExist, strBMMC) = False Then
                                        GoTo errProc
                                    End If

                                    If blnExist = True Then
                                        If blnMutilCheck = False Then
                                            strErrMsg = "提示：" + strKQRQ(k) + strTime + "[" + strKQJL + "]在" + strBMMC + "已经存在，查证！"
                                            GoTo errProc
                                        Else
                                            strErrMsg = "提示：" + strKQRQ(k) + strTime + "在" + strBMMC + "已经休假，查证！"
                                            GoTo errProc
                                        End If
                                    End If
                                Next
